<?php
include_once 'dbcon.php';
session_start();
?>
<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Bootstrap demo</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <link rel="stylesheet" href="./css/mystyle.css">
</head>

<body class="d-flex flex-column min-vh-100">
    <header>
        <?php include_once('./template/navtop.php'); ?>
    </header>

    <main class="mb-auto mt-5 pt-3">
        <div class="container-fluid">
            <div class="d-flex justify-content-center">
                <h1>Profile</h1>
            </div>
            <?php if (!empty($_SESSION['username'])) { ?>
                <div class="container-fluid">
                    <div class="d-flex justify-content-center">
                        <h2>Active Orders</h2>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered align-middle">
                        <tr>
                            <th>Order ID</th>
                            <th>Total</th>
                            <th>Payment ID</th>
                            <th>Order Status</th>
                            <th>View Order Items</th>
                        </tr>
                        <tbody class="table-group-divider">
                            <?php
                            $stmt1 = $db->prepare("SELECT * FROM order_details WHERE order_status='Active' AND user_id=:user_id");
                            $stmt1->bindParam(':user_id', $_SESSION['user_id'], PDO::PARAM_STR);
                            $stmt1->execute();
                            $orders = $stmt1->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($orders as $order) {
                            ?>
                                <tr>
                                    <td><?php echo $order['id'] ?></td>
                                    <td><?php echo $order['total'] ?></td>
                                    <td><?php echo $order['payment_id'] ?></td>
                                    <td><?php echo $order['order_status'] ?></td>
                                    <td>
                                        <form action="profileViewItems.php" method="post">
                                            <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
                                            <input type="submit" name="viewItems" value="View Items">
                                        </form>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
                    </table>
                </div>
                <div class="container-fluid">
                    <div class="d-flex justify-content-center">
                        <h2>Order History</h2>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered align-middle">
                        <tr>
                            <th>Order ID</th>
                            <th>Total</th>
                            <th>Payment ID</th>
                            <th>Order Status</th>
                            <th>View Order Items</th>
                        </tr>
                        <tbody class="table-group-divider">
                            <?php
                            $stmt1 = $db->prepare("SELECT * FROM order_details WHERE order_status='Inactive' AND user_id=:user_id");
                            $stmt1->bindParam(':user_id', $_SESSION['user_id'], PDO::PARAM_STR);
                            $stmt1->execute();
                            $orders = $stmt1->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($orders as $order) {
                            ?>
                                <tr>
                                    <td><?php echo $order['id'] ?></td>
                                    <td><?php echo $order['total'] ?></td>
                                    <td><?php echo $order['payment_id'] ?></td>
                                    <td><?php echo $order['order_status'] ?></td>
                                    <td>
                                        <form action="profileViewItems.php" method="post">
                                            <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
                                            <input type="submit" name="viewItems" value="View Items">
                                        </form>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
                    </table>
                </div>
            <?php } ?>
        </div>
        <?php if (!empty($_SESSION['username'])) { ?>
            <div class="container-fluid"><a href="./logout.php">Logout</a></div>
        <?php } ?>

    </main>


    <footer>
        <?php include_once('./template/navbot.php'); ?>
    </footer>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
</body>

</html>